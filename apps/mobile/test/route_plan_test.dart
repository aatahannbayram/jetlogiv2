...
void main() {
  ...
  test('planDayRoute: Güney demo kümesinde ağ düşünce tohum geometri kullanılır', () async {
    ...
    expect(route.stops.first.taskIds, ['t1']);
    expect(route.highlight.length, greaterThan(2));
    final toAhmet = route.legFor('t1');
    expect(toAhmet, isNotNull);
    expect(toAhmet!.meters, 1472);
    expect(toAhmet.points.length, greaterThan(5));
    expect(toAhmet.points.first.latitude, closeTo(38.1476, 1e-2));
    expect(toAhmet.points.last.latitude, closeTo(38.1512, 1e-2));
    expect(route.legFor('t3')?.meters, 1486);
  });
